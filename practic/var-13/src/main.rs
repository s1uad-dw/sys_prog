
use plotters::prelude::*;
fn main() {
    let mut a:f32 = 3.4;
    let b:f32 = 4.6;
    println!(" x  y    f");
    let mut i = 0;
    while a <= b{
        println!("{:2} {:.1} {:.3}", i, a, get_value(a));
        a+=0.1;
        i +=1;
    }
    let zilihudmain:f32 = find_integral_by_rectangles(&3.4, &4.6);
    println!("метод прямоугольников: {:.3}", zilihudmain);
    println!("Метод симпсона: {:.3}", num_3());


    drow()
}

fn get_value(x:f32)-> f32{
    let a:f32 = 2.;
    return 20.0*a.powf(-(x*x)/2.0);
}

fn find_integral_by_rectangles(&a:&f32, &b:&f32,)-> f32{
    let mut result:f32 = 0.0;

    let n:i32 = 1000000;
    let h:f32 = (b-a) / n as f32;
    println!("h={h}, n={n}");

    for k in 0..n{
        let xk:f32 = a + (k as f32 * h);
        let step_result:f32 = get_value(xk-h/2.0)*h;
        result = result + step_result;
    }
    return result
}

fn num_3()-> f32{
    let a:f32 = 3.4;
    let b:f32 = 4.6;
    let multiplier1 = (b-a)/(3.0*16.0);
    let mut multiplier2 = get_value(3.4);
    let interval:f32 = (b-a)/16.0;

    for i in 1..16{
        multiplier2+=get_value(a+i as f32 * interval) * (2.0 + (i % 2) as f32 *2.0);
    }

    multiplier2+=get_value(b);

    multiplier1*multiplier2
}

fn drow(){
    let root = BitMapBackend::new("plot.png", (640, 480)).into_drawing_area();
    root.fill(&WHITE).unwrap();

    let mut chart = ChartBuilder::on(&root)
        .caption("y = 20.0*e^(-(x^2)/2.0)", ("sans-serif", 30))
        .set_label_area_size(LabelAreaPosition::Left, 50)
        .set_label_area_size(LabelAreaPosition::Bottom, 50)
        .build_cartesian_2d(0.0..10.0, 0.0..60.0).unwrap();

    chart.configure_mesh().draw().unwrap();

    let step = (4.6-3.4)/16.0;
    chart.draw_series(LineSeries::new(
        (0..=100)
        .map(|x| (x as f64 / (3.4*step), 20.0 * (-((x as f64 /(3.4*step)) * (x as f64 / (3.4*step))) / 2.0).exp())),
        &RED,
    )).unwrap();
}