...
fn read_data(file:&mut File) -> Vec<Participant>{
    ...
    for _i in 0..buffer[offset]{
        ...
        data.push(
            Participant { club: (club), surname: (surname), start_time: (start_time), finish_time: (finish_time) }
        )
    }
    return data
}
